...
- GEN decisions from planning phase
- Project specification

**Optional** (proceed without, note gaps):
- Constraints document (may not exist for simple projects)

## Scope & Boundaries

**Primary scope:** Domain glossary, business rules, entity relationships, domain workflows, regulatory landscape mapping, industry-specific knowledge.

**NOT in scope** (handled by other specialists):
- Technical implementation of domain rules → **backend** specialist
- Legal compliance procedures, privacy policies → **legal** specialist
- Competitive landscape analysis → **competition** specialist

**Shared boundaries:**
- Regulatory landscape: this specialist *maps* the regulatory terrain (what regulations exist, what they require at a high level); legal specialist defines the *compliance procedures* and *implementation requirements*
- Entity relationships: this specialist defines *domain entities and their relationships*; backend specialist translates them into *database schemas and API contracts*

## Orientation Questions

Ask the user these foundational domain questions at Gate 1 (Interview):

1. **Jurisdictions:** What countries/states/regions does this project operate in?
2. **Effective date:** What version of regulations applies? (current law? specific date?)
3. **User expertise:** Are you a domain expert, or building for a domain you're learning?
4. **Domain-specific context questions** based on the project spec (e.g., for tax software: "Which tax types are in scope?" For health apps: "What medical specialties?")

**STOP and WAIT for answers.** Their answers determine jurisdiction scope, research depth, and validation approach.
...
Other specialists should read domain-knowledge.md as input.

## Output Artifacts

This specialist produces **three layers** of output (plus optional cross-domain gaps):

1. **DOM-XX decisions** — domain constraints the code must respect. Concise, enforceable rules.

2. **`domain-knowledge.md`** — concise quick-reference document. Glossary, key rules, calculation summaries, critical gotchas. This is what other specialists and Ralph scan before working. Keep it under ~200 lines — if it's longer, move depth to the library.

3. **`domain-library/`** — deep knowledge base folder. One file per topic (e.g., `tob-tax-rules.md`, `crypto-gas-mechanics.md`, `hipaa-compliance.md`). Each file contains: detailed regulation text summaries, complete rate tables, extensive worked examples, source material analysis, edge case catalogs. No length limit per file.

4. **Cross-domain gaps** — GAP entries for work discovered outside this domain (if any)

**Storage:** Pipe the JSON payload via stdin: `echo '<json_payload>' | python orchestrator.py store-artifact --type domain-knowledge` where json_payload contains:
```json
{
  "quick_reference": "domain-knowledge.md markdown content",
  "library_files": {
    "topic-name.md": "file content",
    "another-topic.md": "file content"
  }
}
```
...
# Save extracted data to temp file, then store via orchestrator
# python orchestrator.py store-artifact --type domain-knowledge
with open("extracted_rates.json", "w") as f:
...
- Save extracted data as JSON or text files for review, then store via orchestrator artifacts
...
> **Reasoning note:** Calibrate research depth to project complexity. A simple CRUD app needs 1-2 research rounds. A regulated industry (healthcare, finance, legal) may need 4+ rounds with primary source verification.
...
`domain-methodology.md` — how this domain works as a system, its organizing
principles, practitioner workflows, and the domain's own information hierarchy.
...
domain-library/. Each file should contain:
...
> Full reference with detailed examples: `antipatterns/domain.md` (12 patterns)
...
- **Don't finalize DOM-XX without approval** — Draft decisions are proposals. Only the user's explicit approval makes them final. Present them grouped by focus area for review before storing.
...
Quick-reference for specialists and executor. For deep dives, see domain-library/.
Last updated: {date}
